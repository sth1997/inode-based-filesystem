...
TEST(inode_test, directory)
{
    ...
    int dir0BlockNum = rootInode->createInode("dir0", superBlock, inodeBitmap, dataBitmap, directory);
    int dir1BlockNum = rootInode->createInode("dir1", superBlock, inodeBitmap, dataBitmap, directory);
    int data0BlockNum = rootInode->createInode("data0", superBlock, inodeBitmap, dataBitmap, file);
    ASSERT_EQ(*rootInode->size, 16 * 3);
    Inode* dir1Inode = new Inode(dir1BlockNum, false);
    int dir2BlockNum = dir1Inode->createInode("dir2", superBlock, inodeBitmap, dataBitmap, directory);
    ASSERT_EQ(*superBlock->inodeNumbers, 5);
    ASSERT_EQ(dir2BlockNum, *superBlock->inodeBeginBlock + 4);
    int data1BlockNum = dir1Inode->createInode("data1", superBlock, inodeBitmap, dataBitmap, file);
    delete rootInode;
    delete dir1Inode;
    ASSERT_EQ(Inode::nameToInodeNumber("data0", rootBlockNum - *superBlock->inodeBeginBlock, superBlock, inodeBitmap), 3);
    ASSERT_EQ(Inode::nameToInodeBlockNumber("dir2", rootBlockNum - *superBlock->inodeBeginBlock, superBlock, inodeBitmap), -1);
    ASSERT_EQ(Inode::nameToInodeBlockNumber("dir2", dir1BlockNum - *superBlock->inodeBeginBlock, superBlock, inodeBitmap), 4 + *superBlock->inodeBeginBlock);

    ASSERT_EQ(Inode::absolutePathToInodeNumber("/", superBlock, inodeBitmap), 0);
    ASSERT_EQ(Inode::pathToInodeNumber("dir1/dir2", 0, superBlock, inodeBitmap), dir2BlockNum - *superBlock->inodeBeginBlock);
    ASSERT_EQ(Inode::absolutePathToInodeNumber("/dir1/dir2", superBlock, inodeBitmap), dir2BlockNum - *superBlock->inodeBeginBlock);
    ASSERT_EQ(Inode::pathToInodeNumber("dir1/data1", 0, superBlock, inodeBitmap), data1BlockNum - *superBlock->inodeBeginBlock);
    ASSERT_EQ(Inode::pathToInodeNumber("dir0/dir2", 0, superBlock, inodeBitmap), -1);
    Inode::link("dir1/data1", "dir1/dir2/data2", 0, superBlock, inodeBitmap, dataBitmap);
    int data2InodeNumber = Inode::pathToInodeNumber("dir1/dir2/data2", 0, superBlock, inodeBitmap);
    ...
    dir1Inode->deleteInode(string("data1"), superBlock, inodeBitmap, dataBitmap);
    ASSERT_EQ(dir1Inode->nameToInodeNumber("data1", superBlock, inodeBitmap), -1);
    delete dir1Inode;
    ASSERT_EQ(Inode::pathToInodeNumber("data1", dir1BlockNum - *superBlock->inodeBeginBlock, superBlock, inodeBitmap), -1);
